...
function SettingHirer() {
  const token = localStorage.getItem("token")

  // Hirer-specific API endpoints
  const fetchUrl = `${BACKEND_URL}/api/hirer`
  const updateUrl = `${BACKEND_URL}/api/updatehirer`
      ...
      <div className="flex-grow container mx-auto px-4 py-8 max-w-6xl">
        ...
        <Tabs defaultValue="profile" className="w-full">
          ...
          <TabsContent value="personal">
            <PersonalInfoSection token={token} fetchUrl={fetchUrl} updateUrl={updateUrl} userRole="hirer" />
          </TabsContent>
